...
object HiveExternalCatalog {
  ...
  private def getSchemaFromTableProperties(metadata: CatalogTable): StructType = {
    ...
    if (schema.isDefined) {
      ...
      StructType(
        DataType.fromJson(schema.get).asInstanceOf[StructType].fields.map(field => {
          if (field.name.equalsIgnoreCase("geometry")) {
            if (metadata.storage.properties.get("geometry_type").isEmpty) {
              StructField(field.name, PointUDT, true)
            } else {
              StructField(field.name,
                if (metadata.storage.properties("geometry_type").equalsIgnoreCase("point")) PointUDT else GeometryUDTPublic,
                true)
            }
          } else {
            field
      ...
      if (numSchemaParts.isDefined) {
        ...
        StructType(
          DataType.fromJson(parts.mkString).asInstanceOf[StructType].fields.map(field => {
            if (field.name.equalsIgnoreCase("geometry")) {
              if (metadata.storage.properties.get("geometry_type").isEmpty) {
                StructField(field.name, PointUDT, true)
              } else {
